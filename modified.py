...
Friend1 = User(name="John",age=23,rating=7,stat=False)
Friend2 = User(name="Jack",age=25,rating=8.7,stat=True)
            ...
            if inp == '1':
                ...
                while True:
                    try:
                        ...
                        continue
                    break
                current_stat = Spy.stat_mess[item - 1]            # Updating current status
                break
            elif inp == '2':
                current_stat = raw_input(colored("Type new status :",'grey','on_white',attrs=['bold']))
                Spy.stat_mess.append(current_stat)               # Appending new status to old status list
                break
            else:
                print colored("Invalid input",'red')
